...
<%
	...
	if (is_mine.equals("N")) {
		...
		try {
			...
			password_check = sqlSession.selectOne("BBSCommentMapper.checkPassword", comment);
		} catch (Exception e) {
	...
	int delete_count = 0;
	try {
		delete_count = sqlSession.delete("BBSCommentMapper.doDelete", comment_id);
	} catch (Exception e) {
		logger.error(e.getMessage());
	}
	...
	if (delete_count < 1) {
		sqlSession.close();
		helper.printJsonRt("삭제된 댓글이 없습니다.");
		return;
	}
