...
<%
	...
	Member member = new Member(0, user_id, user_pw, user_name, email, tel,
			postcode, addr1, addr2, null, null);
	...
	// 조회 결과가 0보다 크다면 중복된 아이디임
	if (id_count > 0) {
		// 처리를 종료해야 하므로 DB접속 해제
		sqlSession.close();
		helper.redirect(null, "이미 사용중인 아이디입니다.");
		return;
	}
	...
	if (email_count > 0) {
		// 처리를 종료해야 하므로 DB접속 해제
		sqlSession.close();
		helper.redirect(null, "이미 사용중인 이메일입니다.");
		return;
	}
