...
<%
	...
	/** 작성 내용을 Beans 객체로 구성 */
	BBSDocument document = new BBSDocument(document_id, 0, writer_name, writer_pw, email, subject, content,
			ip_address, 0, null, null);
	...
	if (is_mine.equals("N")) {
		...
		try {
			// 데이터 조회 결과 호출
			password_check = sqlSession.selectOne("BBSDocumentMapper.checkPassword", document);
		} catch (Exception e) {
		...
		try {
			saved_document = sqlSession.selectOne("BBSDocumentMapper.getDocumentItem", document_id);
		} catch (Exception e) {
			logger.error(e.getMessage());
		}
	...
	try {
		// 데이터 수정하기
		update_count = sqlSession.update("BBSDocumentMapper.doEdit", document);
	} catch (Exception e) {
		logger.error(e.getMessage());
	}
	...
	try {
		//
		savedFileList = sqlSession.selectList("BBSFileMapper.getFileList", document_id);
	} catch (Exception e) {
		logger.error(e.getMessage());
	}
	...
	for (int i = 0; i < fileList.size(); i++) {
		...
		BBSFile file = new BBSFile(0, document_id, f.getFileDir(), f.getFileName(), f.getFileSize(),
