	...
	<div class="container">
			...
			<table class='table table-hover'>
				...
				<tbody>
							...
							if (documentList.size() > 0) {
								...
								for (int i = 0; i < documentList.size(); i++) {
									...
									try {
										...
										// 하루 전의 시간보다 게시물 작성일시가 더 이후라면 신규글로 인식함
										if (stamp < itemStamp) {
											is_new = true;
										}
									} catch (Exception e) {
					...
					<tr>
						...
						<td><a href='read.jsp?document_id=<%=item.getId()%>'> <%=item.getSubject()%></a>
		...
		<div class='clearfix'>
			...
			<div class='pull-right'>
				<a href="write.jsp" class="btn btn-primary"> <i
					class="glyphicon glyphicon-pencil"></i> 글쓰기
				</a>
			</div>
		</div>
		...
		<nav class='text-center'>
			<ul class="pagination">
				...
				%>
				<li><a
					href="list.jsp?page=<%=pageHelper.getPrevPage()%>&keyword=<%=keyword%>"><span
				...
				%>
				<li><a
					href="list.jsp?page=<%=pageHelper.getNextPage()%>&keyword=<%=keyword%>"><span
